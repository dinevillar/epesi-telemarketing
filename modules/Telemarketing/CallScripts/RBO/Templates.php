...
class Telemarketing_CallScripts_RBO_Templates extends RBO_Recordset
{
    ...
    function display_name($record, $nolink = false)
    {
        return Utils_RecordBrowserCommon::create_linked_label_r(
            Telemarketing_CallScripts_RBO_Templates::TABLE_NAME, 'Name', $record, $nolink);
    }
    ...
    function QFfield_content($form, $field, $label, $mode, $default, $args, $rb_obj)
    {
        ...
        if (isset($submitValues['pages']) && $submitValues['pages']) {
            ...
            if ($mode == 'view' || $mode == 'edit') {
                $pages = Telemarketing_CallScriptsCommon::get_callscript_page($rb_obj->record['id']);
        ...
        $paginator = Telemarketing_CallScriptsCommon::get_paginator_by_pages_html(
        ...
        if ($mode == 'add' || $mode == 'edit') {
            ...
            $excluded_target_fields = Telemarketing_CallScriptsCommon::get_excluded_contact_fields();
            $placeholders = array(
        ...
        $ck_module_path = '/modules/' . Telemarketing_CallScriptsInstall::module_name() . '/callscript-ck/';
