...
//        if ($mode!='add' && !$this->record[':active'] && !Base_AclCommon::i_am_admin()) return $this->back();
        ...
        if ($show_actions!==false) {
            if ($mode=='view') {
                ...
                Base_ActionBarCommon::add('save', __('Save'), $form->get_submit_form_href(). " id='rb_save_action'");
                Base_ActionBarCommon::add('delete', __('Cancel'), $this->create_back_href(). " id='rb_cancel_action'");
            }
            //Utils_ShortcutCommon::add(array('esc'), 'function(){'.$this->create_back_href_js().'}');
        }
