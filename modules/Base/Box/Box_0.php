...
//          $this->set_reload(true);
    ...
    public function body()
    {
        ...
        $base_box_ini_common = ModuleManager::call_common_methods('base_box_ini');
        foreach ($base_box_ini_common as $m => $ini_common) {
            foreach ($ini_common as $k => $ini_val) {
                $ini_val['common'] = true;
                $ini_file[$k] = $ini_val;
            }
        }
        ...
        foreach ($ini_file as $tag => $opts) {
            ...
            if (isset($opts['common']))
                $containers[$tag]['common'] = $opts['common'];
            if (isset($opts['orientation']))
                $containers[$tag]['orientation'] = $opts['orientation'];
        }
...
//        print_r($containers);
        ...
        foreach ($containers as $k => $v) {
            ...
            if (isset($v['common']) && $v['common'] && isset($v['orientation'])) {
                $commons[$v['orientation']][$k] = $content;
            } else {
                $theme->assign($k, $content);
            }
            ob_end_clean();
        }
        $theme->assign('left_commons', $commons['left']);
        $theme->assign('right_commons', $commons['right']);
