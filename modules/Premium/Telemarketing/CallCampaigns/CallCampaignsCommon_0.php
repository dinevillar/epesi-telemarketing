...
class Premium_Telemarketing_CallCampaignsCommon extends ModuleCommon
{
    ...
    public static function submit_call_campaign($values, $mode)
    {
        if ($mode == 'view' || $mode == 'display' || $mode == 'edit') {
            ...
            if (ModuleManager::is_installed("Premium/Telemarketing/Dialer") >= 0 && in_array($my["id"], $values['telemarketers'])) {
                Base_ActionBarCommon::add(
                    Base_ThemeCommon::get_template_file("Premium/Telemarketing/Dialer", 'icon.png'),
                    __('Dialer'),
                    Base_BoxCommon::create_href(
                        self::module_name(),
                        'Premium_Telemarketing_Dialer', 'body', array($values)
                    )
                );
            }
        }
        if ($mode === "editing" || $mode === "adding") {
            load_js("modules/" . Premium_Telemarketing_CallCampaignsInstall::module_name() . "/js/campaigns.js");
    ...
    public static function update_settings($campaign_id, $settings = false)
    {
        ...
        $settings_table = Premium_Telemarketing_CallCampaignsInstall::SETTINGS_TABLE;
    ...
    public static function campaign_format($record, $nolink = false)
    {
        ...
        if (!$nolink) {
            $t = ' < a href = "' . Utils_RecordBrowserCommon::create_record_href(
                    Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME, $record['id']
                ) . '" > ' . $t . '</a > ';
        }
        return $t;
    }
    ...
    {
        $campaign_rbo = new Premium_Telemarketing_CallCampaigns_RBO_Campaigns();
