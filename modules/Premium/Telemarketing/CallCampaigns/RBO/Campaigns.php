 ...
 * @Time: 7:40 AM
 */
class Premium_Telemarketing_CallCampaigns_RBO_Campaigns extends RBO_Recordset
{
    ...
    function fields()
    {
        ...
        $call_script = new RBO_Field_Select(
            ...
            'Name',
            array('Premium_Telemarketing_CallCampaignsCommon', 'callscript_template_crits')
        );
    ...
    function QFfield_lead_list($form, $field, $label, $mode, $default, $args, $rb_obj)
    {
        ...
        if ($mode === 'add' || $mode === 'edit') {
            $criteria = Utils_RecordBrowserCommon::get_records(
                ...
                Premium_Telemarketing_CallCampaignsCommon::crm_criteria_callcampaign_crits()
            );
            $criteria_options = [
                '' => '---'
            ];
            foreach ($criteria as $criterion) {
                $criteria_options[$criterion['id']] = Utils_RecordBrowserCommon::create_default_linked_label(
                    Premium_Criteria_RBO_Rules::TABLE_NAME, $criterion['id'], true
                );
            }
            $form->addElement(
                'select',
            ...
            if (ModuleManager::is_installed('Telemarketing/CallCampaigns/Premium/ListManager') >= 0) {
                Premium_Telemarketing_CallCampaigns_ListManagerIntegrationCommon::QFfield_lead_list($form, $field, $label, $mode, $default, $args, $rb_obj);
            }
            $form->addElement(
                'text',
                $field,
...
//            $values['title'] = __('Follow-up') . ': ' . Premium_Telemarketing_CallCampaignsCommon::campaign_format($record, true);
//            $values['status'] = 0;
//
//            if ($action != 'none') {
//                $values['subject'] = __('Follow-up') . ': ' . Premium_Telemarketing_CallCampaignsCommon::campaign_format($record, true);
//                $values['follow_up'] = array(self::table_name(), $record['id'], Premium_Telemarketing_CallCampaignsCommon::campaign_format($record, true));
