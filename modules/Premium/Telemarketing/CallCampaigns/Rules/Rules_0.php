...
class Premium_Telemarketing_CallCampaigns_Rules extends Module
{
    public $theme;
    public $form;
    public $mode;
    ...
    public function campaign_rules()
    {
        ...
        if ($this->mode == 'admin') {
            ...
            $rules = Premium_Telemarketing_CallCampaigns_RulesCommon::get_rules($this->campaign['id']);
            foreach ($rules as $i => $rule) {
                $theme_rules[$i] = $this->get_campaign_rule_template($i, $rule, '');
            }
        }
    ...
    private function get_merge_field_content($lb_id = false)
    {
        ...
        $theme->assign('call_campaign_merge_fields',
            Premium_Telemarketing_CallCampaigns_RulesCommon::get_call_campaign_merge_fields());

        if (ModuleManager::is_installed('Premium/Telemarketing/Products') >= 0) {
            $theme->assign('product_merge_fields',
                Premium_Telemarketing_CallCampaigns_RulesCommon::get_product_merge_fields());
        }

        $contact_merge_fields = Premium_Telemarketing_MergeFieldsCommon::get_fields("contact",
            Premium_Telemarketing_CallScriptsCommon::get_excluded_contact_fields());
        $theme->assign('contact_merge_fields', $contact_merge_fields);

        $emp_merge_fields = Premium_Telemarketing_MergeFieldsCommon::get_fields("contact",
            Premium_Telemarketing_CallScriptsCommon::get_excluded_contact_fields(), 'emp');
        ...
        $theme->assign('static_texts', array(
            ...
            'merge_fields' => __('Merge Fields'),
            'merge_open' => Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID,
            'merge_close' => Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
            'copy' => __('Copy')
        ));
    ...
    public function add_rule()
    {
        ...
        if ($this->mode == 'admin') {
            ...
            Premium_Telemarketing_CallCampaigns_RulesCommon::update_rules($this->campaign['id'], false, $new_rule);
        }
    }
    ...
    public function save_rules($values)
    {
        ...
        foreach ($per_rule as $index => $submitted_rule) {
            ...
            foreach ($submitted_rule as $key => $value) {
                if (Premium_Telemarketing_CallCampaigns_RulesCommon::starts_with($key, 'callcampaign_rules')) {
                    if (Premium_Telemarketing_CallCampaigns_RulesCommon::ends_with($key, '_type')) {
                        $rule['type'] = $value;
                    } else if (Premium_Telemarketing_CallCampaigns_RulesCommon::ends_with($key, '_condition_0')) {
                        $rule['condition'] = $value . $rule['condition'];
                    } else if (Premium_Telemarketing_CallCampaigns_RulesCommon::ends_with($key, '_action_0')) {
        ...
        if ($this->mode == 'admin') {
            ...
            foreach ($rules as $index => $rule) {
                Premium_Telemarketing_CallCampaigns_RulesCommon::update_rules($this->campaign['id'], $index, $rule);
            }
        }
    ...
    public function delete_rule($index)
    {
        if ($this->mode == 'admin') {
            ...
            Premium_Telemarketing_CallCampaigns_RulesCommon::delete_rule($this->campaign['id'], $index);
        }
    }
}
