...
class Premium_Telemarketing_CallCampaigns_RulesCommon extends ModuleCommon
{
    ...
    public static function rules_addon_label($campaign)
    {
        if (
            ...
            Base_AclCommon::check_permission(Premium_Telemarketing_CallCampaignsInstall::manage_permission)
        ) {
    ...
    {
        $call_campaign_merge_fields = Premium_Telemarketing_MergeFieldsCommon::get_fields(
    ...
    {
        $product_merge_fields = Premium_Telemarketing_MergeFieldsCommon::get_fields(
    ...
    {
        $text = Premium_Telemarketing_MergeFieldsCommon::parse_fields(
            Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME,
            $campaign,
            $text,
            'call_campaign'
        ...
        if ($disposition) {
            if (isset($disposition['disposition'])) {
                ...
                $text = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID .
                    'call_campaign_disposition' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                    $dispositions[$disposition['disposition']], $text);
            }
            if (isset($disposition['talk_time'])) {
                $text = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID .
                    'call_campaign_talk_time' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
            ...
            if (isset($disposition['call_back_time'])) {
                ...
                $text = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID .
                    'call_back_date_time' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                    $cbt, $text);
            }
        }
        if ($values) {
            if (isset($values['phone'])) {
                $text = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID .
                    'call_campaign_called_phone' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                    $record[$values['phone']], $text);

                $text = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID .
                    'call_campaign_called_phone_type' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                    ucwords(str_replace('_', ' ', $values['phone'])), $text);
            }
        }

        if ($replace_all) {
            $text = preg_replace('/\\' . Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID . '.*?' . '\\' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID . '/', '', $text);
        }
        return $text;
    }
    ...
    {
        $rules_table = Premium_Telemarketing_CallCampaigns_RulesInstall::RULES_TABLE;
        if ($index === false) {
    ...
    public static function match_rules($campaign, $type = false, $action = false, $condition = false)
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ($type && $action && $condition) {
                $add = $rule['type'] == $type &&
                    self::starts_with($rule['action'], $action) &&
                    self::starts_with($rule['condition'], $condition);
            } else if (!$type && $action && $condition) {
                $add = self::starts_with($rule['action'], $action) &&
                    self::starts_with($rule['condition'], $condition);
            } else if ($type && !$action && $condition) {
                $add = $rule['type'] == $type &&
                    self::starts_with($rule['condition'], $condition);
                ...
                $add = self::starts_with($rule['condition'], $condition);
            } else if (!$type && $action && !$condition) {
                $add = self::starts_with($rule['action'], $action);
            } else if ($type && !$action && !$condition) {
                $add = $rule['type'] == $type;
                ...
                switch ($rule['action_send']) {
                    ...
                    case 0:
                        ...
                        foreach ($to as $recipient) {
                            $r = trim($recipient);
                            $r = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID . 'email' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                                $record['email'], $r);
                            $r = str_replace(Premium_Telemarketing_MergeFieldsCommon::MERGE_OPEN_ID . 'emp_email' . Premium_Telemarketing_MergeFieldsCommon::MERGE_CLOSE_ID,
                                $me['email'], $r);
                ...
                switch ($rule['action_add']) {
                    ...
                    case 0:
                        $defaults = array(
                            ...
                            'related' => array(Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME . '/' . $campaign['id'])
                        );
