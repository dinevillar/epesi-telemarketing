...
{/foreach}
{php}
	$this->assign('form_data_keys', array_keys($this->_tpl_vars['form_data']));
				...
				<tr>
                    ...
                    {foreach key=k item=f from=$fields name=fields}
                        {if $f.type!="multiselect"}
                            ...
                            {/if}
                            {if $f.element == "lead_list" && $action != 'view'}
								<tr>
                                ...
                                {foreach item=fk from=$form_data_keys}
                                    {if strpos($fk, 'lead_list_') === 0}
										<tr id="{$form_data[$fk].name}" class="lead_list"
										    style="display: none;">
											<td class="label">
                                                {$form_data[$fk].label}
											</td>
											<td class="data">{$form_data[$fk].html}</td>
										</tr>
                                    {/if}
                                {/foreach}
                            {else}
