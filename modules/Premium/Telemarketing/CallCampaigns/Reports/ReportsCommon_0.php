...
class Premium_Telemarketing_CallCampaigns_ReportsCommon extends ModuleCommon
{
    public static function menu()
    {
        if (Acl::check_permission(Premium_Telemarketing_CallCampaignsInstall::manage_permission)) {
            return
                array(
                    _M('Reports') => array(
                        '__submenu__' => 1, '__weight__' => 7,
                        _M('Call Campaigns by Telemarketer') => array('__weight__' => 1, 'report_mode' =>
                            Premium_Telemarketing_CallCampaigns_Reports::MODE_TELEMARKETER),
                        _M('Call Campaigns') => array('report_mode' => Premium_Telemarketing_CallCampaigns_Reports::MODE_CALL_CAMPAIGNS),
                    )
                );
        } else {
            return array();
    ...
    public static function count_dispositions(
        ...
        $tz = false)
    {
        $dispositions_rbo = new Premium_Telemarketing_CallCampaigns_Dispositions_RBO_Status();
        $crits = array(
