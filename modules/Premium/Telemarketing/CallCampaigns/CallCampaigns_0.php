...
class Premium_Telemarketing_CallCampaigns extends Module
{

    public function body()
    {
        $me = CRM_ContactsCommon::get_my_record();
        $campaigns_tb = new Premium_Telemarketing_CallCampaigns_RBO_Campaigns();
        ...
        if (Base_AclCommon::check_permission(Premium_Telemarketing_CallCampaignsInstall::manage_permission) &&
            ...
            $bl_type = Base_ThemeCommon::get_template_file(
                Premium_Telemarketing_CallCampaigns_DispositionsInstall::module_name(), 'blacklist.png'
            );
    ...
    public function blacklisted_records()
    {
        if (ModuleManager::is_installed("Telemarketing/CallCampaigns/Dispositions") >= 0) {
            ...
            $blacklist->register_processing_callback(array('Premium_Telemarketing_CallCampaigns_DispositionsCommon', 'submit_blacklist'));
    ...
    public function campaign_settings($campaign, $mode = "admin")
    {
        ...
        if ($form->validate()) {
            ...
            if ($mode == 'admin') {
                ...
                Premium_Telemarketing_CallCampaignsCommon::update_settings($campaign['id'], $values);
            }
        ...
        if ($mode == 'admin') {
            ...
            $default_rules = Premium_Telemarketing_CallCampaignsCommon::get_settings($campaign['id']);
        }
    ...
    public function cc_campaigns_actions($record, $gb_row)
    {
        ...
        if (ModuleManager::is_installed("Premium/Telemarketing/Dialer") >= 0 && in_array($my["id"], $record['telemarketers'])) {
            $icon = Base_ThemeCommon::get_template_file("Premium_Telemarketing_Dialer", 'icon-small.png');
            $tag_attrs = Base_BoxCommon::create_href(
                $this, "Premium_Telemarketing_Dialer", 'body', [
                    $record
                ]
            );
