var Dialer = {
    ...
    check_optimal: function () {
        ...
        new Ajax.Request('modules/Premium/Telemarketing/Dialer/checkoptimal.php', {
            method: 'post',
            parameters: param,
    ...
    save_campaign: function () {
        ...
        jQuery('#dialer_hidden_talktime').val(Dialer.totaltalktime)
        Dialer.submit_form()
    },
    add_log: function () {
    ...
    set_talk_time: function () {
        if (Dialer.is_web_phone_active() && WebPhone.talk_time) {
            jQuery('#dialer_hidden_talktime').val(WebPhone.talk_time)
        } else {
            var talktime = Math.floor((new Date().getTime() - Dialer.talktimestart.getTime()) / 1000)
            Dialer.totaltalktime += talktime
            jQuery('#dialer_hidden_talktime').val(Dialer.totaltalktime)
        }
    },
    end_call: function () {
    ...
    init: function () {
        Dialer.talktimestart = new Date()
        if (parseInt(Dialer.currentrecord) !== parseInt(Dialer.calledrecord)) {
            Dialer.totaltalktime = 0
            Dialer.called = 0
        }
        jQuery(document).ready(function () {

            jQuery('#dialer_auto_call_countdown').hide()
            Dialer.autocallcdvar = false
    ...
    autocallcount: function (cb, duration) {
        ...
        jQuery('#dialer_auto_call_countdown').show('fast', function () {
            Dialer.autocallcdvar = setInterval(function () {
                if (--duration) {
