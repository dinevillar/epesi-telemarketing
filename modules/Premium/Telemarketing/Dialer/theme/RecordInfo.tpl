...
{if $main_page}
	<table class="Utils_RecordBrowser__table" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
            ...
            {*CORE UPDATE: Relocated, related buttons*}
			<td>
				<div>
					<a {$form_save_href} style="display: none;" id="save_form_link">Save</a>
                    {foreach item=p from=$custom_new}
						<span class="epesi_big_button" style="margin:1px;"{$p.atts} onclick="{$p.onclick}">
                            <a href='javascript:void(0);'><img src="{$p.image}" width='16' height='16'/></a>
                        </span>
                    {/foreach}
				</div>
			</td>
				...
				<tr>
					<td colspan="{$cols}">
						<table cellpadding="0" cellspacing="0" border="0"
						       class="longfields {if $action == 'view'}view{else}edit{/if}" style="border-top: none;">
                            {foreach key=k item=f from=$longfields name=fields}
								<tr>
									<td class="label long_label">{$f.label}{if $f.required}*{/if}</td>
									<td class="data long_data {if $f.type == 'currency'}currency{/if}"
									    id="_{$f.element}__data">
										<div style="position:relative;">
                                            {if $f.error}{$f.error}{/if}{$f.html}{if $action == 'view'}&nbsp;{/if}
										</div>
									</td>
