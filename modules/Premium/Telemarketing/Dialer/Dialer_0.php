...
class Premium_Telemarketing_Dialer extends Module
{
    private $campaign;
    ...
    public function body($campaign = false)
    {
        ...
        if (!$campaign && $this->isset_shared_module_variable('selected-campaign')) {
            $campaign = $this->get_shared_module_variable('selected-campaign');
        }
        $this->init_campaign_select_dialog(!$campaign);
        if ($campaign) {
            $this->set_shared_module_variable('selected-campaign', $campaign);
            if (is_numeric($campaign)) {
                $campaign = Utils_RecordBrowserCommon::get_record(Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME,
                    $campaign);
            }
            if ($campaign[':active'] == 1) {
                ...
                load_css('modules/' .
                    Premium_Telemarketing_CallScripts::module_name() .
                ...
                if ($cl_rules = Premium_Telemarketing_CallCampaigns_RulesCommon::match_rules(
                ...
                if ($campaign['product']) {
                    $products_rbo = new Premium_Telemarketing_Products_RBO_Products();
                ...
                if (!$this->isset_module_variable('dialer_in_call')) {
                    ...
                    Base_ActionBarCommon::add('settings', __('Campaign Settings'),
                        Utils_RecordBrowserCommon::create_record_href(
                            Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME, $campaign['id']) .
                        ' style="float:right"'
                    );

                }

                $next_record = Premium_Telemarketing_DialerCommon::get_next_record($campaign);
                if (!$next_record || empty($next_record)) {
                    ...
                    $location = Premium_Telemarketing_ContactLocalTimeCommon::get_contact_location($this->record);
                    ...
                    $this->form->addElement(
                        ...
                        Premium_Telemarketing_ContactLocalTimeCommon::local_time(
                            $this->record,
                            $this->record_type
                        )
                    );
                    ...
                    if (!$this->isset_module_variable('dialer_in_call')) {
                        ...
                        $count_next = Premium_Telemarketing_DialerCommon::count_remaining_records($this->campaign);
                        $allow_skip = Premium_Telemarketing_CallCampaignsCommon::get_settings($this->campaign['id'], 'allow_skip');
    ...
    {
        $active_campaigns = Premium_Telemarketing_CallCampaignsCommon::get_active_campaigns();
    ...
    public function redirect_to_new_campaign()
    {
        ...
        Base_BoxCommon::push_module("Utils/RecordBrowser", 'view_entry', array(
            ...
            array(Premium_Telemarketing_CallCampaigns_RBO_Campaigns::TABLE_NAME));
        return true;
    }
    ...
    public function set_disposition()
    {
        ...
        $my = CRM_ContactsCommon::get_my_record();
        if (empty($disp_recs)) {
            Premium_Telemarketing_CallCampaigns_DispositionsCommon::clear_lock(
            ...
            if ($rec['locked_to'] != $my['id']) {
                ...
                Utils_RecordBrowserCommon::update_record(
                    Premium_Telemarketing_CallCampaigns_Dispositions_RBO_Status::TABLE_NAME, $rec['id'], $rec
                );
            }
    ...
    public function set_form_defaults($values = array())
    {
        ...
        if ($this->record['work_phone']) {
            $phones['work_phone'] = Premium_Telemarketing_DialerCommon::format_phone($this->record, 'work_phone');
        }
        if ($this->record['home_phone']) {
            $phones['home_phone'] = Premium_Telemarketing_DialerCommon::format_phone($this->record, 'home_phone');
        }
        if ($this->record['mobile_phone']) {
            $phones['mobile_phone'] = Premium_Telemarketing_DialerCommon::format_phone($this->record, 'mobile_phone');
        }
        if ($this->record['phone']) {
            $phones['phone'] = Premium_Telemarketing_DialerCommon::format_phone($this->record, 'phone');
        }
        ...
        if ($this->record_type == 'contact' && count($phones) > 1) {
            ...
            if (!$this->isset_module_variable('dialer_in_call')) {
                $prio['work_phone'] = Premium_Telemarketing_CallCampaignsCommon::get_settings($this->campaign['id'], 'prio_work');
                $prio['home_phone'] = Premium_Telemarketing_CallCampaignsCommon::get_settings($this->campaign['id'], 'prio_home');
                $prio['mobile_phone'] = Premium_Telemarketing_CallCampaignsCommon::get_settings($this->campaign['id'], 'prio_mobile');
                $high_key = 4;
                $discon_phones = array();
    ...
    {
        $auto_scroll = Premium_Telemarketing_CallCampaignsCommon::get_settings(
            $this->campaign['id'],
            'auto_scroll'
        );
        $auto_scroll_speed = Premium_Telemarketing_CallCampaignsCommon::get_settings(
            $this->campaign['id'],
            'auto_scroll_speed'
        );
        if ($auto_scroll) {
    ...
    public function set_auto_call()
    {
        ...
        $auto_call_delay = Premium_Telemarketing_CallCampaignsCommon::get_settings(
            $this->campaign['id'],
            'auto_call_delay'
        );
    ...
    {
        $filter = Premium_Telemarketing_CallCampaignsCommon::get_settings(
            $this->campaign['id'],
    ...
    public function build_dialer_form()
    {
        ...
        $this->form->addElement(
            ...
            'dialer',
            __('Select Dialer'),
            Premium_Telemarketing_DialerCommon::get_dialing_methods(),
    ...
    public function build_dialer_theme($values = array())
    {
        ...
        $campaign['icon'] = Base_ThemeCommon::get_template_file(
            Premium_Telemarketing_CallCampaigns::module_name(),
            'icon.png'
        );
    ...
    public function get_record_info()
    {
        ...
        $record_rb->set_expandable_rows(false);
    ...
    public function get_callscript_content()
    {
        ...
        $callscript_paginator = Premium_Telemarketing_CallScriptsCommon::get_paginator_html($this, $callscript);
        $first_content = Premium_Telemarketing_MergeFieldsCommon::parse_fields(
            $this->record_type,
            $this->record,
            $callscript['content']
        );
        ...
        $callscript_pages = Premium_Telemarketing_CallScriptsCommon::get_callscript_page($callscript);
        if (count($callscript_pages)) {
            foreach ($callscript_pages as $k => $v) {
                $target_content = Premium_Telemarketing_MergeFieldsCommon::parse_fields(
                    $this->record_type,
                    $this->record,
                    $v['content']
                );
...
//                break;
            ...
            case 'save':
                ...
                break;
            case 'add_log':
                Premium_Telemarketing_DialerCommon::add_call_log(
                    $this->disposition,
    ...
    public function call_record($values)
    {
        ...
        $phone = Premium_Telemarketing_DialerCommon::fix_phone_number(
            $this->record[$values['phone']],
            $this->record
        );
        eval_js(Premium_Telemarketing_DialerCommon::get_dial_code_js($phone, $values['dialer']));
        $phone = Premium_Telemarketing_DialerCommon::format_phone($this->record, $values['phone'], true);
        $methods = Premium_Telemarketing_DialerCommon::get_dialing_methods();
        Premium_Telemarketing_DialerCommon::add_call_log(
            $this->disposition,
            __("Called") . " " . $this->record_type . " at {$phone} using " .
            $methods[$values['dialer']] . ' dialer.'
        );
        if ($called_rules = Premium_Telemarketing_CallCampaigns_RulesCommon::match_rules(
            $this->campaign, 'Record', false, 'Called')
        ) {
            foreach ($called_rules as $called_rule) {
                $return = Premium_Telemarketing_DialerCommon::process_rule_action(
                    $called_rule,
                    $this->campaign,
                    $this->record,
                    $this->record_type,
    ...
    public function save($values)
    {
        ...
        Utils_RecordBrowserCommon::update_record(
            Premium_Telemarketing_CallCampaigns_Dispositions_RBO_Status::TABLE_NAME, $this->disposition['id'], $this->disposition
        );
        ...
        Premium_Telemarketing_DialerCommon::add_call_log($this->disposition, Utils_BBCodeCommon::optimize($log));

        if ($flag_rules = Premium_Telemarketing_CallCampaigns_RulesCommon::match_rules(
            ...
            foreach ($flag_rules as $flag_rule) {
                if (Premium_Telemarketing_CallCampaigns_RulesCommon::starts_with($flag_rule['action'], 'Add')) {
                    $add_rules[] = $flag_rule;
                } else if (Premium_Telemarketing_CallCampaigns_RulesCommon::starts_with($flag_rule['action'], 'Remove')) {
                    $remove_rules[] = $flag_rule;
                } else {
                    Premium_Telemarketing_DialerCommon::process_rule_action(
                        $flag_rule,
                        $this->campaign,
                        $this->record,
                        $this->record_type,
