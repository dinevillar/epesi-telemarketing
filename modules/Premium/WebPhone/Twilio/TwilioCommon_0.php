...
class Premium_WebPhone_TwilioCommon extends ModuleCommon
{

    public static function store_config_href()
    {
        return array('module' => 'Premium_WebPhone_Twilio', 'func' => 'admin', 'args' => array());
    }
    ...
    public static function sms_method()
    {
        return array(
            'name' => __('Twilio SMS'),
            'settings_callback' => array('Premium_WebPhone_TwilioCommon', 'sms_settings_form'),
            'send_callback' => array('Premium_WebPhone_TwilioCommon', 'sms_send')
        );
    }
    ...
    public static function sms_send($recipients, $message, $count = 1)
    {
        ...
        try {
            $phone_mapping_rbo = new Premium_WebPhone_Twilio_RBO_PhoneMappings();
    ...
    {
        $pm_rbo = new Premium_WebPhone_Twilio_RBO_PhoneMappings();
        ...
        if ($pm && $pm['phone_number']) {
            ...
            if (Utils_RecordBrowserCommon::get_access($pm_rbo::TABLE_NAME, 'edit', $pm)) {
                ...
                $href = Base_BoxCommon::create_href(null, 'Utils/RecordBrowser', 'view_entry', array('edit', $pm['id'], array()), array($pm_rbo::TABLE_NAME));
        ...
        if (Utils_RecordBrowserCommon::get_access($pm_rbo::TABLE_NAME, 'add')) {
            ...
            $href = Base_BoxCommon::create_href(null, 'Utils/RecordBrowser', 'view_entry', array('add', NULL, array()), array($pm_rbo::TABLE_NAME));
    ...
    {
        return Premium_WebPhoneCommon::default_dialer($title);
    }

    public static function dialer_js($title)
    {
        return Premium_WebPhoneCommon::default_dialer_js($title);
    }
