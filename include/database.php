...
return $ret;
    ...
    public static function replace_values($sql, $inputarr = false)
    {
        $sql = self::TypeControl($sql, $inputarr);
        $sqlarr = explode('?', $sql);
        ...
        foreach ($inputarr as $arr) {
            ...
            while (list(, $v) = each($arr)) {
                ...
                if ($typ == 'string')
                    ...
                    $sql .= $v ? self::$ado->true : self::$ado->false;
                else if ($typ == 'object') {
                    if (method_exists($v, '__toString')) $sql .= self::$ado->qstr($v->__toString());
                    else $sql .= self::$ado->qstr((string)$v);
                } else if ($v === null)
                    $sql .= 'NULL';
                else
                    $sql .= $v;
            ...
            if (isset($sqlarr[$i])) {
                $sql .= $sqlarr[$i];
                if ($i + 1 != sizeof($sqlarr)) self::$ado->outp_throw("Input Array does not match ?: " . htmlspecialchars($sql), 'Execute');
            } else if ($i != sizeof($sqlarr))
                self::$ado->outp_throw("Input array does not match ?: " . htmlspecialchars($sql), 'Execute');
